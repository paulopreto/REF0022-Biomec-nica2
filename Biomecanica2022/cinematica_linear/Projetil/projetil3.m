% altura inicial maior que altura final
function projetil3(s0,v0,angulo)
...
texto = ['alcance ate a mesma altura de lancamento: ' num2str(Stotal)];
disp(texto)
...
t2 = sqrt(2 * (s0 + h) / g);
texto = ['tempo da altura maxima ate chao: ' num2str(t2)];
disp(texto)
...
texto = ['alcance total: ' num2str(Stotal)];
disp(texto)
...
% plot(t,yy3)
% hold on
% text(t(tamanho),yy3(tamanho) + 0.5,num2str(angulo))
% plot(t,yy3,'.')
...
plot(sx,yy3)
hold on
text(sx(tamanho),yy3(tamanho) + 0.5,num2str(angulo))
