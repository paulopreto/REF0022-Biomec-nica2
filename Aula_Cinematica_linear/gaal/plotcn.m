function [C,h]=plotcn(f,xrange,yrange,npts,color,ncurvas,flag)
%	plotcn(f,[rmin rmax])  
%       plota as curvas de níveis f(x,y)=c, em que f é uma 
%       expressão simbólica que envolve duas variáveis x e y 
%	     Usa como domínio a região definida em coordenadas polares
%       por rmin<r<rmax.
%
%	plotcn(f,[xmin xmax],[ymin ymax])  
%       plota as curvas de níveis f(x,y)=c, em que f é uma 
%       expressão simbólica que envolve duas variáveis x e y 
%	     Usa como domínio a região definida por
%       xmin<x<xmax e ymin<y<ymax
%
%  plotcn(f,[xmin xmax],[ymin ymax],npts,'c',nc)
%       o mesmo que o anterior, com uma discretização de npts
%       na cor 'c' e com nc curvas ou nos níveis dados pelo vetor nc
%
%       written by Reginaldo J. Santos DMAT/ICEx/UFMG on 6 November 2002
%
%	Exemplos:
%       syms x y, plotcn(x^2+y^2,[-1,1],[-1,1]) 
%       syms x y, plotcn(x^2-y^2,[0,1])         
...
colormap('mycolor');
if nargin==6
   [C,h]=contourf(x,y,z,ncurvas,color);
   clabel(C,h);
elseif nargin==7
   [C,h]=contour(x,y,z,ncurvas,color);
else
   contourf(x,y,z,color);
end
grid('off');
%xlabel('x'); ylabel('y');
