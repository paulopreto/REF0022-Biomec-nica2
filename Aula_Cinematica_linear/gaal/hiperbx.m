function hiperbx(a,b,P,X0,cor)
%hiperbx(a,b) plots the ellipse with center in the origin ( x^2/a^2-y^2/b^2=1).
%             desenha a elipse centro na origem ( x^2/a^2-y^2/b^2=1).
%
%hiperbx(a,b,P) plots the ellipse with center in the origin ( x1^2/a^2-y1^2/b^2=1),
%                    where [x;y]=P*[x1;y1].
...
%
%	Examples:
%	    hiperbx(3,2,[1/sqrt(2),1/sqrt(2);-1/sqrt(2),1/sqrt(2)],[1;2])
%      hiperbx(3,2,[1/sqrt(2),1/sqrt(2);-1/sqrt(2),1/sqrt(2)])
%      hiperbx(3,2)
if nargin==2 
   P=eye(2);
end;
...
if nargin > 2
  ...
  if nargin > 3
    ...
    X01=P*X0;
    xx1=xx1+X0(1);
    lineseg(-1.5*a*W1+X01,X01,'k-',1)
    plavec(X01,1.5*a*W1,'k-','x"',1)
    lineseg(X01-1.5*b*W2,X01,'k-',1)
    plavec(X01,1.5*b*W2,'k-','y"',1)
    lineseg(X01,P*[x0,0]','k:')
    lineseg(X01,P*[0,y0]','k:')
    lineseg(O,P*[x0,0]','k-',1)
    lineseg(O,P*[0,y0]','k-',1)
%    tex([-cosphi*0.4-senphi*y0,-senphi*0.4+cosphi*y0],sym(X0,2,1));
%    tex([cosphi*x0+senphi*0.2,senphi*x0-cosphi*0.2],sym(X0,1,1));
  end
  X=P*[x1;y1];
  XX=P*[xx1;y1];
else
...
axiss;
axisc;
lineseg(X01+1.5*P*[a;b],X01+1.5*P*[-a;-b],'b:',1);
lineseg(X01+1.5*P*[-a;b],X01+1.5*P*[a;-b],'b:',1);
%axis('off')
