...
%
%       Exemplos:
%           paraby(2,[1/sqrt(2);1/sqrt(2)],[1;2])
%           paraby(2,[1/sqrt(5);2/sqrt(5)])
%           paraby(2)
clf
hold on
O=[0,0];
...
%  plavec(-3*W1,6*W1,'k-','x`',1);
%  plavec(O,9/(4*mp)*W2,'k-','y`',1)
  plavec(O,W1,'k-','')
  plavec(O,W2,'k-','')
  if nargin == 3
    X0=X0(:); X0=double(X0);
...
%      plavec(-2*W2+X01,4*W2,'k-','y"',1)
      lineseg(X01-2*W2,X01,'k-',1)
%      plavec(X01,1.5*W1,'k-','x"',1)
%      plavec(X01,1.5*W2,'k-','y"',1)
%      lineseg(-1.5*W2+X01,X01,'k-',1)
    else
    ...
    if (p>-1/4)
      ...
      lineseg(X01,X01+W1/(4*p),'k-',1)
      lineseg(X01+1.5*p*4*W1,X01,'k-',1)
...
%    plavec(X01-3*W1,6*W1,'k-','x"',1)
%    plavec(X01,9/(4*mp)*W2,'k-','y"',1)
    lineseg(X01,P*[x0;0],'k:')
    lineseg(X01,P*[0;y0],'k:')
    lineseg(O,P*[x0;0],'k-',1)
    lineseg(O,P*[0;y0],'k-',1)
%    tex([-cosphi*0.4-senphi*(y0-0.2),-senphi*0.4+cosphi*(y0-0.2)],num2str(y0));
